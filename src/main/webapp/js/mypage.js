		...
		$(document.body).on('click', '.zzal-menu02 #subs-btn', function(event) {
			$.getJSON(contextRoot + '/collect/subslist.json', {'mno': no}, function(result) {
				if(result.data){
					...
					$('#my-collection02 .editerbtn').text('구독')
				}
			})
...
//		location.href = 'someonedetail.html?cono=' + $(this).attr('data-no')+'&mno='+$(this).attr('data-memNo')
//		event.preventDefault()
//	});	
	$(document.body).on('click', '#my-collection03 .mycollectlist', function(event) {
		location.href = 'collectdetail.html?cono=' + $(this).attr('data-no')
		event.preventDefault()
	});
	// 짤강 편집 클릭시 

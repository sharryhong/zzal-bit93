...
-- 댓글 좋아요
DROP TABLE IF EXISTS REPLY_MEMB RESTRICT;
...
CREATE TABLE REPLY (
	...
	RE_LIKE  INTEGER    NULL     COMMENT '댓글좋아요 갯수', -- 댓글좋아요 갯수
	REPORT   BOOLEAN    NULL     COMMENT '신고', -- 신고
	REPARENT INTEGER    NULL     COMMENT '대댓글번호', -- 대댓글번호
	REPACNT  INTEGER    NULL     COMMENT '대댓글 갯수', -- 대댓글 갯수
...
-- 알림
CREATE TABLE NOTICE (
	NONO   INTEGER     NOT NULL COMMENT '알림코드', -- 알림코드
	MNO    INTEGER     NULL     COMMENT '회원코드', -- 회원코드
	ZZNO   INTEGER     NULL     COMMENT '짤강의 코드', -- 짤강의 코드
	RNO    INTEGER     NULL     COMMENT '댓글 코드', -- 댓글 코드
	CONO   INTEGER     NULL     COMMENT '컬렉션일련번호', -- 컬렉션일련번호
	dmno   INTEGER     NULL     COMMENT '새 컬럼', -- 새 컬럼
	YNN    BOOLEAN     NULL     DEFAULT false COMMENT '확인여부', -- 확인여부
	NoType VARCHAR(20) NULL     COMMENT '알림유형', -- 알림유형
...
-- 알림
ALTER TABLE NOTICE
	ADD CONSTRAINT PK_NOTICE -- 알림 기본키
		PRIMARY KEY (
			NONO -- 알림코드
		);

ALTER TABLE NOTICE
	MODIFY COLUMN NONO INTEGER NOT NULL AUTO_INCREMENT COMMENT '알림코드';

-- 댓글 좋아요
CREATE TABLE REPLY_MEMB (
	RNO   INTEGER NOT NULL COMMENT '댓글 코드', -- 댓글 코드
	MNO   INTEGER NOT NULL COMMENT '회원코드', -- 회원코드
	REPNO INTEGER NULL     COMMENT '부모댓글번호' -- 부모댓글번호
)
COMMENT '댓글 좋아요';

-- 댓글 좋아요
ALTER TABLE REPLY_MEMB
	ADD CONSTRAINT PK_REPLY_MEMB -- 댓글 좋아요 기본키
		PRIMARY KEY (
			RNO, -- 댓글 코드
			MNO  -- 회원코드
		);
...
-- 알림
ALTER TABLE NOTICE
	ADD CONSTRAINT FK_ZZAL_LECT_TO_NOTICE -- 짤강의 -> 알림
		FOREIGN KEY (
			ZZNO -- 짤강의 코드
		)
		REFERENCES ZZAL_LECT ( -- 짤강의
...
-- 알림
ALTER TABLE NOTICE
	ADD CONSTRAINT FK_REPLY_TO_NOTICE -- 댓글 -> 알림
		FOREIGN KEY (
			RNO -- 댓글 코드
		)
		REFERENCES REPLY ( -- 댓글
...
-- 알림
ALTER TABLE NOTICE
	ADD CONSTRAINT FK_COLCT_TO_NOTICE -- 컬렉션 -> 알림
		FOREIGN KEY (
			CONO -- 컬렉션일련번호
		)
		REFERENCES COLCT ( -- 컬렉션
...
-- 알림
ALTER TABLE NOTICE
	ADD CONSTRAINT FK_MEMB_TO_NOTICE -- 회원 -> 알림
		FOREIGN KEY (
			MNO -- 회원코드
		)
		REFERENCES MEMB ( -- 회원
...
-- 댓글 좋아요
ALTER TABLE REPLY_MEMB
	ADD CONSTRAINT FK_REPLY_TO_REPLY_MEMB -- 댓글 -> 댓글 좋아요
		FOREIGN KEY (
			RNO -- 댓글 코드
		)
		REFERENCES REPLY ( -- 댓글
...
-- 댓글 좋아요
ALTER TABLE REPLY_MEMB
	ADD CONSTRAINT FK_MEMB_TO_REPLY_MEMB -- 회원 -> 댓글 좋아요
		FOREIGN KEY (
			MNO -- 회원코드
		)
		REFERENCES MEMB ( -- 회원
